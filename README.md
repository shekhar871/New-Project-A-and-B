...
| Training script (Grand Finals) | `train_adversarial.py` (defender-only GRPO) + offline corpus generator.                                            |
...
- **Full audit:** `[CALCULATIONS_REFERENCE.md](./CALCULATIONS_REFERENCE.md)` — every formula, bound, and code pointer for **AgentGuard-Gym**.
...
### Grand Finals: end-to-end RL training (defender-only GRPO)

Minimum requirement from the Opening Ceremony: a working training script (TRL/Unsloth) + evidence of real training.
...
This produces checkpoints under `./checkpoints/` (and logs if you enable W&B).
